...
class TournamentManager:
    ...
    def tournament_management(self, tournament_id=None, str=None):
        ...
        options = {
            "1": {"value": self.tournament_resumption, "text": "Reprendre le tournoi"} if str == "started" else {"value": self.start_tournament, "text": "Démarrer le tournoi"},
            "2": {"value": self.main_controller.player_manager.add_player_to_tournament_screen, "text": "Ajouter un joueur"} if str != "started" else None,
            "3": {"value": self.main_controller.tournament_manager.classement_screen, "text": "Classement"},
            "4": {"value": self.main_controller.tournament_manager.matches_history, "text": "Recap matchs"},

            "0": {"value": self.current_tournament, "text": "Retour"} if str == "started" else {"value": self.show_tournaments_to_start, "text": "Retour"},
        }
    ...
    def current_tournament(self):
        ...
        tournaments_lst = TournamentHelper.get_started_tournament_lst(self, "round_lst", self.main_controller.tournament_db)
        back = {"0": {"value": self.main_controller.menu_manager.tournaments_manager, "text": "Retour"}}
        options = {}
        for tournament in tournaments_lst:
    ...
    def start_tournament(self, tournament_id, index_start=1):
        ...
        DatabaseHelper.update_end_date_tournament(self.tournament.end_date, tournament_id, self.main_controller.tournament_db)
    ...
    def round_process(self, index, tournament_id):
        ...
        DatabaseHelper.update_tournament_round(tournament_id, round_serialized, self.main_controller.tournament_db)
        self.match_process(index, round_obj.match_lst, tournament_id)
        round_obj.end_timestamp = round_helper_obj.set_round_time()
        DatabaseHelper.update_end_time_round(round_obj.end_timestamp, round_obj.number, tournament_id, self.main_controller.tournament_db)

    def match_process(self, index, match_lst, tournament_id):
        for match in match_lst:
            result_match = self.set_results_matches(index, match)
            DatabaseHelper.update_match_and_player_score(str(tournament_id), index, result_match, self.main_controller.tournament_db)
    ...
    def tournament_resumption(self, tournament_id):
        ...
        for round in tournament["round_lst"]:
            ...
            DatabaseHelper.update_end_time_round(round["end_timestamp"], round["number"], tournament_id, self.main_controller.tournament_db)
    ...
    def classement_screen(self, tournament_id):
        ...
        for player in sorted_players_ranking_list:
            ...
            new_player_obj = {'first_name': player_obj['first_name'], 'last_name': player_obj['last_name'], 'score': player['score']}
    ...
    def matches_history(self, tournament_id):
        ...
        for round in round_lst:
            new_match_lst = []
            for match in round['match_lst']:
                player_objs_lst = DatabaseHelper.pull_database_lst([match[0]['player_id'], match[1]['player_id']], self.main_controller.player_db)
